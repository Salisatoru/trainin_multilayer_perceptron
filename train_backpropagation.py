...
def load_cross_pat_data():
    data = []
    with open('cross.pat') as f:
        for line in f:
            parts = line.strip().split()
            inputs = list(map(float, parts[:2]))
            outputs = list(map(int, parts[2:]))
            data.append((inputs, outputs))
    return data
def train_network(network, data, learning_rate, momentum, epochs):
    # initialize previous weight updates tracking for momentum
    prev_weight_updates = { 'input_hidden': [[0] * network.hidden_size for _ in range(network.input_size)],
                               'hidden_output': [[0] * network.ourput_size for _ in range(network.hidden_size)]}
    for epoch in range (epochs):
        for inputs, expected_outputs in data:
            hidden_activations, outputs = network.feedforward(inputs)
